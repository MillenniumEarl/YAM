...
import RendererIPCHandler from "./ipc";
...
const handlerWrapper: IRendererIPCHandler = {
  ...
  send: (c: string, ...args: unknown[]) => handler.send(c, ...args),
  // eslint-disable-next-line @typescript-eslint/ban-types
  receive: (c: string, f: Function) => handler.receive(c, f),
  invoke: (c: string, ...args: unknown[]) => handler.invoke(c, ...args)
};
...
contextBridge.exposeInMainWorld("Handler", handlerWrapper);
contextBridge.exposeInMainWorld("Logger", loggerWrapper);
contextBridge.exposeInMainWorld("Dialog", dialogWrapper);
